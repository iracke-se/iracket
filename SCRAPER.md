...
- **Pacing**: profixio sits behind Cloudflare rate limiting (verified 2026-09-15: ~7 requests in 5s → HTTP 429 for minutes; a steady 1 request / 2s never trips it). Each popup costs 2 requests (history + matches), so the defaults are one tab (`SCRAPER_PYTHON_CONCURRENCY=1`), 3s between popups (`SCRAPER_PYTHON_POPUP_DELAY=3`) and genders run one after the other (`SCRAPER_PYTHON_PARALLEL_GENDERS=false`). A full month takes ~13h; it is a once-a-month overnight job, and a slow run that finishes beats a fast one that returns 9 players
- **Throttle handling**: a 429 shows up as a list page / popup with no rows. Those are retried with 60→120→300s backoff; 5 consecutive failures pause every tab for 10 min (max 3 cooldowns), then the run aborts. The script exits non-zero when it processed under 90% of the players the list pages advertised, so the run is marked `failed` instead of "completed" with a handful of rows. Coverage numbers are stored in the run's `step_data.coverage` and checked by `scraper:health`. The women's list advertises the men's page links; pages after the first short one are skipped as past the end
