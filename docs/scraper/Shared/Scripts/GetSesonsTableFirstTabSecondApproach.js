function GetSesonsTableFirstTabSecondApproach() {
            ...
            if (data[i] !== undefined && data[i + 1] !== undefined && data[i + 2] !== undefined) {
                if (data[i].className === '' && data[i + 1].className === '' && data[i + 2].className === '') {
                    if (Object.keys(round).length > 0) {
                        round.Details = detailList; arr.push(round);
                        round = {};
                        detailList = [];
                    }
                    ...
                    round = {}
                    round.Round = temp.Round; round.Organiser = data[i].innerText;
                    round.Date = temp.Date; detailList = [];
                }
                else {
                    try {
                        ...
                        let details = {
                            Player1: cols[1].innerText,
                            Player2: cols[3].innerText,
