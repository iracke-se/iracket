                ...
                <a href="{{ route('my-rankings.index') }}" wire:navigate class="flex items-center justify-between px-4 py-3.5 border-b border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                    ...
                    <div class="flex items-center gap-2">
                        @if($manualPointsDelta != 0)
                            <span class="flex flex-col items-center px-3 py-1 bg-zinc-200 dark:bg-zinc-700 rounded-full leading-tight">
                                <span class="text-xs font-bold text-zinc-900 dark:text-white">{{ number_format($currentRankingPoints) }} {{ __('user-player-show.pts') }}</span>
                                <span class="text-[10px] text-zinc-500 dark:text-zinc-400">{{ __('user-player-show.current_points') }}</span>
                            </span>
                            <span class="flex flex-col items-center px-3 py-1 bg-accent text-white rounded-full leading-tight">
                                <span class="text-xs font-bold">{{ number_format($currentRanking?->points ?? 0) }} {{ __('user-player-show.pts') }}</span>
                                <span class="text-[10px] text-white/80">{{ __('user-player-show.official_points') }}</span>
                            </span>
                        @elseif($currentRanking || $currentRankingPoints > 0)
                    ...
                    <div class="mt-4 flex flex-wrap gap-2 justify-center">
                        @if($manualPointsDelta != 0)
                            <div class="flex flex-col items-center px-4 py-2 bg-zinc-200 dark:bg-zinc-700 rounded-full">
                                <span class="text-sm font-bold text-zinc-900 dark:text-white">{{ number_format($currentRankingPoints) }}</span>
                                <span class="text-xs text-zinc-500 dark:text-zinc-400">{{ __('user-player-show.current_points') }}</span>
                            </div>
                        @endif
                        <div class="flex flex-col items-center px-4 py-2 bg-accent rounded-full">
                            <span class="text-sm font-bold text-white">{{ number_format($currentRanking?->points ?? 0) }}</span>
                            <span class="text-xs text-white/80">{{ __('user-player-show.official_points') }}</span>
                        </div>
                    </div>
                @endif
            </div>
