<div class="mb-6">
    ...
    @else
        <div class="bg-zinc-100 dark:bg-zinc-800 rounded-xl overflow-hidden">
            ...
            <div class="space-y-0" x-data="{ openRanking: null }">
                @foreach($rankingsHistory as $ranking)
                    @php $monthMatches = $matchesByMonth->get($ranking->year . '-' . $ranking->month, collect()); @endphp
                    <div wire:key="ranking-{{ $ranking->id }}" class="border-b border-zinc-200 dark:border-zinc-700 last:border-0">
                        <button
                            @click="openRanking = openRanking === {{ $ranking->id }} ? null : {{ $ranking->id }}"
                        ...
                        >
                            ...
                            <svg :class="{ 'rotate-180': openRanking === {{ $ranking->id }} }" class="w-4 h-4 ml-1 text-zinc-400 transition-transform flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                            </svg>
                        </button>

                        <div x-show="openRanking === {{ $ranking->id }}" style="display: none;">
