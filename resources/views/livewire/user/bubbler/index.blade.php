<div class="max-w-2xl mx-auto">
    <h1 class="text-2xl font-bold text-white mb-6">{{ __('Bubbler') }}</h1>

    <!-- Tabs -->
    <div class="flex gap-2 mb-6">
        <button
            wire:click="setTab('ladies')"
            class="flex-1 px-4 py-3 rounded-lg text-sm font-medium transition-colors {{ $activeTab === 'ladies' ? 'bg-accent text-white' : 'bg-zinc-800 text-zinc-300 hover:bg-zinc-700' }}"
        >
            {{ __('Ladies') }}
        </button>
        <button
            wire:click="setTab('men')"
            class="flex-1 px-4 py-3 rounded-lg text-sm font-medium transition-colors {{ $activeTab === 'men' ? 'bg-accent text-white' : 'bg-zinc-800 text-zinc-300 hover:bg-zinc-700' }}"
        >
            {{ __('Men') }}
        </button>
        <button
            wire:click="setTab('clubs')"
            class="flex-1 px-4 py-3 rounded-lg text-sm font-medium transition-colors {{ $activeTab === 'clubs' ? 'bg-accent text-white' : 'bg-zinc-800 text-zinc-300 hover:bg-zinc-700' }}"
        >
            {{ __('Clubs') }}
        </button>
    </div>
    ...
    <div class="text-center mb-4">
        <p class="text-sm text-zinc-400">{{ now()->format('F Y') }}</p>
    </div>

    <!-- Ladies Tab -->
    @if($activeTab === 'ladies')
        <div>
            <h2 class="text-lg font-semibold text-white mb-4">{{ __('Ladies of the Month') }}</h2>

            @if($ladiesRankings->isEmpty())
                <div class="text-center py-12 bg-zinc-800 rounded-xl">
                    <p class="text-zinc-400">{{ __('No rankings yet') }}</p>
                </div>
            @else
                        ...
                        <tbody>
                            @foreach($ladiesRankings as $ranking)
                                <tr class="border-b border-zinc-700 last:border-0">
                                    <td class="px-4 py-3 text-sm text-zinc-400">{{ $ranking->rank }}</td>
                                    <td class="px-4 py-3">
                                        <a href="{{ route('players.show', $ranking->user) }}" wire:navigate class="flex items-center gap-3 hover:text-accent">
                                            ...
                                            @else
                                                <div class="w-8 h-8 rounded-full bg-zinc-700 flex items-center justify-center">
                                                    <span class="text-xs font-medium text-zinc-300">{{ $ranking->user->initials() }}</span>
                                                </div>
                                            @endif
                                            <div>
                                                <div class="text-sm text-white">{{ $ranking->user->name }}</div>
                                                @if($ranking->user->club)
                                                    <div class="text-xs text-zinc-400">{{ $ranking->user->club->name }}</div>
                                                @endif
                                            </div>
                                        </a>
                                    </td>
    ...
    @if($activeTab === 'men')
        <div>
            <h2 class="text-lg font-semibold text-white mb-4">{{ __('Gentlemen of the Month') }}</h2>
    ...
    @if($activeTab === 'clubs')
        <div>
            <h2 class="text-lg font-semibold text-white mb-4">{{ __('Clubs of the Month') }}</h2>
                                ...
                                <tr class="border-b border-zinc-700 last:border-0">
                                    ...
                                    <td class="px-4 py-3">
                                        <a href="{{ route('clubs.show', $ranking->club) }}" wire:navigate class="flex items-center gap-3 hover:text-accent">
                                            ...
                                            @endif
                                            <div>
                                                <div class="text-sm text-white">{{ $ranking->club->name }}</div>
                                                <div class="text-xs text-zinc-400">{{ $ranking->club->members_count ?? 0 }} {{ __('members') }}</div>
                                            </div>
                                        </a>
                                    </td>
