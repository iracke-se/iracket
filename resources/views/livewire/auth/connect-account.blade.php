<div class="flex flex-col gap-6">
        ...
        @if($isActivePlayer)
            ...
            <div>
                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">
                    {{ __('connect.choose_club') }}
                </label>
                <button
                    type="button"
                    wire:click="openClubModal"
                ...
                >
                    <span class="{{ $selectedClubName ? 'text-zinc-900 dark:text-zinc-100' : 'text-zinc-500 dark:text-zinc-400' }}">
                        {{ $selectedClubName ?? __('connect.select_club') }}
                    </span>
    ...
    @if($showClubModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <!-- Background overlay -->
            <div class="fixed inset-0 bg-zinc-500 dark:bg-zinc-900 bg-opacity-75 dark:bg-opacity-75 transition-opacity" wire:click="closeClubModal"></div>
            ...
            <div class="relative bg-white dark:bg-zinc-800 rounded-xl text-left overflow-hidden shadow-xl transform transition-all w-full max-w-sm">
                    ...
                    <div class="px-4 py-3 border-b border-zinc-200 dark:border-zinc-700">
                        <div class="flex items-center justify-between">
                            <h3 class="text-lg font-semibold text-zinc-900 dark:text-white">{{ __('connect.choose_club') }}</h3>
                            <button type="button" wire:click="closeClubModal" class="text-zinc-400 hover:text-zinc-500">
                        ...
                        <div class="mt-3 relative">
                            <input
                                type="text"
                                wire:model.live.debounce.300ms="clubSearch"
                                placeholder="{{ __('connect.search_club') }}"
                    ...
                    <div class="max-h-96 overflow-y-auto">
                        @forelse($clubsGrouped as $letter => $clubs)
                            ...
                            @foreach($clubs as $club)
                                <button
                                    ...
                                    class="w-full px-4 py-3 text-left hover:bg-zinc-50 dark:hover:bg-zinc-700 border-b border-zinc-100 dark:border-zinc-700 last:border-0 {{ $clubId === $club->id ? 'bg-accent/10' : '' }}"
                                >
